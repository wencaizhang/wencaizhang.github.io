...
function mapTocToSidebar (root, collapsable, prefix) {
  ...
  files.forEach((filename, index) => { // 结构化文章
    const file = path.resolve(root, filename) // 方法：将路径或路径片段的序列解析为绝对路径
    const stat = fs.statSync(file) // 文件信息
    // 文件名开头的数字默认作为 order，这里是获取数字类型 order ，因此如果有两个文件开头分别是 0 和 00，则其中一个会被覆盖掉
    // 如果文件名不以数字开头，则把 files.length + index 作为 order
    let order = (/^\d+/.exec(filename) || [files.length + index])[0]
    ...
    if (stat.isDirectory()) { // 是文件夹目录
      const notImgDir = !/images|imgs/.test(filename)
      if (notImgDir) {
        sidebar[order] = {
          title,
      ...
      // 附带有排序效果
      sidebar[order] = './' + prefix + (filename === 'README.md' ? '' : filename).replace('.md', '')
    }
  })
